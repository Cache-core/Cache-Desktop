...
QString b1Style = "QPushButton{font-size: " + QString::number(baseLargeButtonSize) + "px; color:#fff;border:1px solid pink;border-radius:5px;padding-left: 10px;padding-right: 10px;} QPushButton:hover{color:pink;}";
  ...
  if (!m_ui->validationCheckBox->isChecked())
  {
    QMessageBox::warning(
        ...
        tr("You must confirm that you have safely stored the mnemonic seed and understand that the "
           "Cache Team, nor its developers, can restore lost wallets and is not responsible for loss of funds."));
    return;
  }
